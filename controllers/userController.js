...
const handleNewUser = asyncHandler(async (req, res, next) => {
    ...
    if (!fnIn || !lnIn || !emailIn || !passwordIn || !usernameIn /* || !sqIn || !sqaIn */)
        return res.status(400).json({
            message: 'Full name, email, password, Security Question & Answer are required',
        });
...
const emergencyContacts = asyncHandler(async (req, res) => {
    ...
    console.log(result);
    res.status(201).json({
        success: `Emergency Contact ${foundFriendsEmail.email} set for ${foundUserEmail.email}`,
    });
});
module.exports = {
    handleNewUser,
    ...
    handleLogout,
    handleRefreshToken,
    determineRequestType: determineRequestType(),
    getProfile,
    emergencyContacts,
};
